...
final class ProgressSnapshotFactoryTests: XCTestCase {
    ...
    func testStreakFreezeBaseEvaluatorDoesNotReplayServerFrozenDays() throws {
        let baseSummary = ProgressSummary(
            currentStreakDays: 2,
            longestStreakDays: 2,
        ...
        let pendingEvaluation = try evaluateProgressStreakFreeze(
        ...
        XCTAssertEqual(2, pendingEvaluation.currentStreakDays)
        XCTAssertEqual(2, pendingEvaluation.longestStreakDays)
        XCTAssertEqual(1, pendingEvaluation.streakFreeze.availableCredits)
        XCTAssertEqual(11, pendingEvaluation.streakFreeze.balanceUnits)
        XCTAssertEqual([ProgressStreakDay(date: "2026-04-18", state: .pending)], pendingEvaluation.streakDays)

        let reviewedTodayEvaluation = try evaluateProgressStreakFreeze(
            baseSummary: baseSummary,
            baseStreakDays: [],
            activeReviewLocalDates: ["2026-04-18"],
            today: "2026-04-18",
            policy: progressStreakFreezePolicy
        )
        XCTAssertEqual(3, reviewedTodayEvaluation.currentStreakDays)
        XCTAssertEqual(3, reviewedTodayEvaluation.longestStreakDays)
        XCTAssertEqual(1, reviewedTodayEvaluation.streakFreeze.availableCredits)
        XCTAssertEqual(12, reviewedTodayEvaluation.streakFreeze.balanceUnits)
        XCTAssertEqual([ProgressStreakDay(date: "2026-04-18", state: .reviewed)], reviewedTodayEvaluation.streakDays)
    }

    func testStreakFreezeBaseEvaluatorReplacesServerFrozenDayWithLocalReview() throws {
        let baseSummary = ProgressSummary(
            currentStreakDays: 2,
            longestStreakDays: 2,
            hasReviewedToday: false,
        ...
        let evaluation = try evaluateProgressStreakFreeze(
            ...
            activeReviewLocalDates: ["2026-04-17"],
            today: "2026-04-18",
            policy: progressStreakFreezePolicy
        )
        ...
        XCTAssertEqual(20, evaluation.streakFreeze.balanceUnits)
        XCTAssertEqual(
            [
                ProgressStreakDay(date: "2026-04-17", state: .reviewed),
                ProgressStreakDay(date: "2026-04-18", state: .pending),
            ],
            evaluation.streakDays
        )
    ...
    func testStreakFreezeBaseEvaluatorDoesNotReplayDaysAfterFrozenReplacement() throws {
        let baseSummary = ProgressSummary(
            currentStreakDays: 3,
            longestStreakDays: 3,
            hasReviewedToday: false,
            ...
            activeReviewDays: 2,
            streakFreeze: makeTestProgressStreakFreeze(availableCredits: 1, balanceUnits: 12)
        )
        ...
        let evaluation = try evaluateProgressStreakFreeze(
            ...
            baseStreakDays: [
                ...
                ProgressStreakDay(date: "2026-04-19", state: .pending),
            ],
            activeReviewLocalDates: ["2026-04-17", "2026-04-19"],
            today: "2026-04-19",
            policy: progressStreakFreezePolicy
        )

        XCTAssertEqual(4, evaluation.currentStreakDays)
        XCTAssertEqual(4, evaluation.longestStreakDays)
        XCTAssertEqual(2, evaluation.streakFreeze.availableCredits)
        XCTAssertEqual(20, evaluation.streakFreeze.balanceUnits)
        XCTAssertEqual(
            [
                ProgressStreakDay(date: "2026-04-17", state: .reviewed),
                ProgressStreakDay(date: "2026-04-19", state: .reviewed),
            ],
            evaluation.streakDays
        )
    }

    func testStreakFreezeBaseEvaluatorAppliesConsecutiveLocalDatesAfterServerBase() throws {
        let baseSummary = ProgressSummary(
        ...
        let evaluation = try evaluateProgressStreakFreeze(
            baseSummary: baseSummary,
            baseStreakDays: [],
            activeReviewLocalDates: ["2026-04-17", "2026-04-18"],
            today: "2026-04-18",
            policy: progressStreakFreezePolicy
        )

        XCTAssertEqual(202, evaluation.currentStreakDays)
        XCTAssertEqual(202, evaluation.longestStreakDays)
        XCTAssertEqual(2, evaluation.streakFreeze.availableCredits)
        XCTAssertEqual(20, evaluation.streakFreeze.balanceUnits)
        XCTAssertEqual(
            [
