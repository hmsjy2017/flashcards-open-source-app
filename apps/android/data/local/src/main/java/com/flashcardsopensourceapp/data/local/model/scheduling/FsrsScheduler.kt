 ...
 * `apps/backend/src/scheduling/index.ts` and mirrored in
...
// Keep in sync with apps/backend/src/scheduling/index.ts::ReviewableCardScheduleState and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::ReviewableCardScheduleState.
...
// Keep in sync with apps/backend/src/scheduling/index.ts::ReviewHistoryEvent and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::FsrsReviewHistoryEvent.
data class FsrsReviewHistoryEvent(
    val rating: ReviewRating,
    val reviewedAtMillis: Long
)

// Keep in sync with apps/backend/src/scheduling/index.ts::RebuiltCardScheduleState and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::RebuiltCardScheduleState.
...
// Keep in sync with apps/backend/src/scheduling/index.ts::FsrsMemoryState and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::FsrsMemoryState.
private data class FsrsMemoryState(
    val difficulty: Double,
    val stability: Double
)

// Keep in sync with apps/backend/src/scheduling/index.ts::FuzzRange and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::FuzzRange.
private data class FuzzRange(
    val minInterval: Int,
    val maxInterval: Int
)

// Keep in sync with apps/backend/src/scheduling/index.ts::LearningStepResult and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::LearningStepResult.
...
// Keep in sync with apps/backend/src/scheduling/index.ts::DEFAULT_W and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::defaultWeights.
...
// Keep in sync with apps/backend/src/scheduling/index.ts::FUZZ_RANGES and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::fuzzRanges.
...
// Keep in sync with apps/backend/src/scheduling/index.ts::DECAY and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::fsrsDecay.
private val fsrsDecay: Double = -defaultWeights[20]

// Keep in sync with apps/backend/src/scheduling/index.ts::FACTOR and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::fsrsFactor.
...
private class MashGenerator {
    ...
    // Keep in sync with apps/backend/src/scheduling/index.ts::createMash and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::MashGenerator.next(data:).
...
private class AleaGenerator(seed: String) {
    ...
    // Keep in sync with apps/backend/src/scheduling/index.ts::Alea and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::AleaGenerator.init(seed:).
    init {
    ...
    // Keep in sync with apps/backend/src/scheduling/index.ts::Alea.next and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::AleaGenerator.next().
    fun next(): Double {
...
// Keep in sync with apps/backend/src/scheduling/index.ts::roundTo8 and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::roundTo8(value:).
fun roundTo8(value: Double): Double {
    val formattedValue = String.format(Locale.US, "%.8f", value)
    return formattedValue.toDouble()
}

// Keep in sync with apps/backend/src/scheduling/index.ts::clamp and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::clamp(value:min:max:).
...
// Keep in sync with apps/backend/src/scheduling/index.ts::dateDiffInDays and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::dateDiffInDays(lastReviewedAt:now:).
...
// Keep in sync with apps/backend/src/scheduling/index.ts::stateRequiresMemory and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::stateRequiresMemory(state:).
private fun stateRequiresMemory(state: FsrsCardState): Boolean {
    return state != FsrsCardState.NEW
}

// Keep in sync with apps/backend/src/scheduling/index.ts::getIntervalModifier and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::getIntervalModifier(requestRetention:).
...
// Keep in sync with apps/backend/src/scheduling/index.ts::formatSeedNumber and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::formatSeedNumber(value:).
...
// Keep in sync with apps/backend/src/scheduling/index.ts::mapRatingToFsrsGrade and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::mapRatingToFsrsGrade(rating:).
...
// Keep in sync with apps/backend/src/scheduling/index.ts::getStepsForState and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::getStepsForState(settings:state:).
...
// Keep in sync with apps/backend/src/scheduling/index.ts::getCurrentStepIndex and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::getCurrentStepIndex(card:).
private fun getCurrentStepIndex(card: ReviewableCardScheduleState): Int {
    return card.fsrsStepIndex ?: 0
}

// Keep in sync with apps/backend/src/scheduling/index.ts::getLearningStrategyStepIndex and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::getLearningStrategyStepIndex(card:grade:).
...
// Keep in sync with apps/backend/src/scheduling/index.ts::getHardStepMinutes and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::getHardStepMinutes(steps:).
...
// Keep in sync with apps/backend/src/scheduling/index.ts::getLearningStepResult and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::getLearningStepResult(settings:card:grade:).
...
// Keep in sync with apps/backend/src/scheduling/index.ts::initStability and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::initStability(grade:).
private fun initStability(grade: Int): Double {
    return max(defaultWeights[grade - 1], 0.1)
}

// Keep in sync with apps/backend/src/scheduling/index.ts::initDifficulty and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::initDifficulty(grade:).
...
// Keep in sync with apps/backend/src/scheduling/index.ts::meanReversion and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::meanReversion(initialDifficulty:currentDifficulty:).
...
// Keep in sync with apps/backend/src/scheduling/index.ts::linearDamping and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::linearDamping(deltaDifficulty:difficulty:).
...
// Keep in sync with apps/backend/src/scheduling/index.ts::nextDifficulty and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::nextDifficulty(difficulty:grade:).
...
// Keep in sync with apps/backend/src/scheduling/index.ts::forgettingCurve and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::forgettingCurve(elapsedDays:stability:).
...
// Keep in sync with apps/backend/src/scheduling/index.ts::nextRecallStability and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::nextRecallStability(difficulty:stability:retrievability:grade:).
...
// Keep in sync with apps/backend/src/scheduling/index.ts::nextForgetStability and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::nextForgetStability(difficulty:stability:retrievability:).
...
// Keep in sync with apps/backend/src/scheduling/index.ts::getShortTermWeights and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::getShortTermWeights(settings:).
...
// Keep in sync with apps/backend/src/scheduling/index.ts::nextShortTermStability and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::nextShortTermStability(stability:grade:settings:).
...
// Keep in sync with apps/backend/src/scheduling/index.ts::createInitialMemoryState and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::createInitialMemoryState(grade:).
...
// Keep in sync with apps/backend/src/scheduling/index.ts::computeNextShortTermMemoryState and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::computeNextShortTermMemoryState(memoryState:grade:settings:).
...
// Keep in sync with apps/backend/src/scheduling/index.ts::computeNextReviewMemoryState and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::computeNextReviewMemoryState(memoryState:elapsedDays:grade:settings:).
...
// Keep in sync with apps/backend/src/scheduling/index.ts::getFuzzRange and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::getFuzzRange(interval:elapsedDays:maximumInterval:).
...
// Keep in sync with apps/backend/src/scheduling/index.ts::getIntervalSeed and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::getIntervalSeed(now:reps:memoryState:).
...
// Keep in sync with apps/backend/src/scheduling/index.ts::nextInterval and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::nextInterval(stability:elapsedDays:settings:intervalSeed:).
...
// Keep in sync with apps/backend/src/scheduling/index.ts::getMemoryState and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::getMemoryState(card:).
...
// Keep in sync with apps/backend/src/scheduling/index.ts::buildShortTermSchedule and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::buildShortTermSchedule(card:nextMemoryState:rating:now:reps:lapses:settings:nextState:elapsedDays:intervalSeed:).
...
// Keep in sync with apps/backend/src/scheduling/index.ts::buildGraduatedReviewSchedule and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::buildGraduatedReviewSchedule(nextMemoryState:now:reps:lapses:settings:elapsedDays:intervalSeed:).
...
// Keep in sync with apps/backend/src/scheduling/index.ts::buildReviewSuccessSchedule and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::buildReviewSuccessSchedule(now:reps:lapses:settings:elapsedDays:hardMemoryState:goodMemoryState:easyMemoryState:rating:intervalSeed:).
...
// Keep in sync with apps/backend/src/scheduling/index.ts::createEmptyReviewableCardScheduleState and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::createEmptyReviewableCardScheduleState(cardId:).
...
// Keep in sync with apps/backend/src/scheduling/index.ts::computeReviewSchedule and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::computeReviewSchedule(card:settings:rating:now:).
fun computeReviewSchedule(
    card: ReviewableCardScheduleState,
    settings: WorkspaceSchedulerSettings,
    rating: ReviewRating,
...
// Keep in sync with apps/backend/src/scheduling/index.ts::rebuildCardScheduleState and apps/ios/Flashcards/Flashcards/Review/Scheduling/FsrsScheduler.swift::rebuildCardScheduleState(cardId:settings:reviewEvents:).
